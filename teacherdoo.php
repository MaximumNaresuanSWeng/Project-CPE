<?php
	...
	$strSQLteacherstatus = "SELECT * FROM teacher_status WHERE professorID = '".$_SESSION['userID']."' ";
...
<?php
		//echo $objResultteacher['teacherID'];
		//echo "<br>";
		//echo $objResultteacherstatus['professorID'];
		//echo "<br>";
		//echo $_SESSION["teacherID"];
	if($_SESSION['userID'] == $objResultteacherstatus['professorID']){
		//echo "Professor";
		//echo "<br><br>";
		//echo $objResultteacher['teacherID'];
		//echo $objResultteacherstatus['professorID'];
	...
	$strSQLprojectall = "SELECT project.projectID, project.projectnameTH, project.projectnameENG  
	FROM project join teacher_status on project.projectID = teacher_status.projectID
	WHERE teacher_status.professorID = '".$_SESSION['userID']."' ";
	$objQueryprojectall = mysql_query($strSQLprojectall);
	//$objResultprojectall = mysql_fetch_array($objQueryprojectall);

	//$strSQLproject = "SELECT project.projectID, project.projectnameTH, project.projectnameENG 
	//FROM project join teacher_status on project.projectID = teacher_status.projectID
	//join teacher_detail on teacher_status.professorID = '".$_SESSION['userID']."'"; //WHERE teacherID = '".$_SESSION['userID']."' ";
	//$objQueryproject = mysql_query($strSQLproject);
	
     // output data of each row
     while($row1 = mysql_fetch_assoc($objQueryprojectall)) {
         echo "<tr>
		 <td><center>" . $row1["projectID"]. "</center></td>
		 <td><center>" . $row1["projectnameTH"]. "</center></td>
		 <td><center>" . $row1["projectnameENG"]. "</center></td>
			</tr>";
     }
     echo "</table>";
	 	}else {
